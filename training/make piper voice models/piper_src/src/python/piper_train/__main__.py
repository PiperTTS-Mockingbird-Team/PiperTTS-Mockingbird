...
def main():
    ...
    if args.checkpoint_epochs is not None:
        # 1. Standard Interval Checkpoint (Keep all, let external manager cleanup)
        # We use save_top_k=-1 to ensure Lightning doesn't auto-delete history
        callbacks = [ModelCheckpoint(every_n_epochs=args.checkpoint_epochs, save_top_k=-1)]
        
        # 2. "Best Quality" Checkpoint (Always keep the absolute best epoch)
        # Monitors the 'loss_mel_epoch' metric we added to lightning.py
        callbacks.append(ModelCheckpoint(
            monitor="loss_mel_epoch",
            save_top_k=1,
            mode="min",
            filename="best-epoch={epoch}-loss={loss_mel_epoch:.4f}",
            auto_insert_metric_name=False,
            every_n_epochs=1  # Check every epoch for a new best
        ))
        
        trainer.callbacks = callbacks
        _LOGGER.debug(
            "Checkpoints will be saved every %s epoch(s), plus the best model based on Mel Loss", args.checkpoint_epochs
        )
