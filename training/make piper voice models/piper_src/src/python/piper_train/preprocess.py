#!/usr/bin/env python3
"""
Piper Preprocessing Script
==========================
This script prepares raw audio datasets (LJSpeech or Mycroft format) for training with Piper.
Key tasks:
1. Scans metadata and audio files.
2. Normalizes audio (silence removal, resampling).
3. Phonemizes text using espeak-ng or direct codepoints.
4. Generates 'config.json' and 'dataset.jsonl' needed for training.
"""
import argparse
import csv
import dataclasses
...
def main() -> None:
    ...
    # Write config
    # The config.json is essential for the training stage, defining input layers and sample rates.
        ...
        json.dump(
            {
                ...
                # Default inference settings (can be overridden later)
    ...
    # Calculate batch size for multi-processing.
    # [FIXED] Ensure batch_size is at least 1 to avoid 'n must be at least one' error on tiny datasets.
    ...
    # Spin up background worker processes to handle phonemization and audio normalization in parallel.
    processes = [
    ...
    # The 'dataset.jsonl' is the main index file for the training process.
    # Each line is a JSON object representing one audio clip.
...
def phonemize_batch_espeak(
    ...
    """Worker process: Fetches batches from queue_in, phonemizes with eSpeak-ng, and puts results in queue_out."""
    try:
...
def ljspeech_dataset(args: argparse.Namespace) -> Iterable[Utterance]:
    """
    Parser for LJSpeech format datasets.
    Expects metadata.csv in 'filename|transcript' or 'filename|speaker|transcript' format.
    Handles multiple directory structures (wavs/ folder vs root level files).
    """
    ...
    with open(metadata_path, "r", encoding="utf-8") as csv_file:
        ...
        for row in reader:
            ...
            # Systematic search for the WAV file:
            # 1. Try file name exactly as in metadata
            wav_path = metadata_path.parent / filename

            if not wav_path.exists():
                # 2. Try with .wav append (common if names are numbers)
                wav_path = metadata_path.parent / f"{filename}.wav"

            if not wav_path.exists():
                # 3. Try inside wav/ or wavs/ directory
                wav_path = wav_dir / filename

            if not wav_path.exists():
                # 4. Try inside wav/ or wavs/ with .wav append
